class Bullet{
	...
	draw(){
		//当前位置碰撞检测
		var monArr=[...monSet];
		for(var i=0;i<monArr.length;i++){   //遍历所有怪物
			let l=Math.sqrt(Math.pow(Math.abs(this.x-monArr[i].x),2)+Math.pow(Math.abs(this.y-monArr[i].y),2));   //子弹和怪物的距离
			if(l<monArr[i].w*0.5){          //如果子弹和怪物的距离小于怪物宽度的一半，射中
				 monArr[i].death();          //怪物死亡
				 this.death();         //子弹死亡
